...
(async () => {
  ...
  assert.ok(await page.locator('#employeeRequestList').getByText('ตรวจสอบและสั่งพิมพ์').isVisible());
  assert.ok(!(await page.locator('#employeeRequestList').textContent()).includes('รับเข้าระบบออกบิล'));

  await page.locator('#employeeRequestList').getByText('ตรวจสอบและสั่งพิมพ์').click();
  await page.waitForFunction(() => document.querySelector('#employeeRequestPrintPrep')?.textContent.includes('สั่งพิมพ์'));
  const beforePrint = await page.evaluate(() => JSON.parse(localStorage.getItem('invoices') || '[]'));
  assert.strictEqual(beforePrint.length, 0);
  const prepText = await page.locator('#employeeRequestPrintPrep').textContent();
  assert.ok(prepText.includes('บริษัท อีทูอี เนทีฟ จำกัด'));
  assert.ok(prepText.includes('อ.เมือง จ.ชลบุรี 20000'));

  await page.locator('#employeeRequestPrintPrep').getByRole('button', { name: 'สั่งพิมพ์', exact: true }).click();
  ...
  await page.click('nav button[onclick="showPage(\'employeeRequests\')"]');
  await page.waitForFunction(() => document.querySelector('#employeeRequestList')?.textContent.includes('ยังไม่มีคำขอจากพนักงานที่รอตรวจสอบและสั่งพิมพ์'));
