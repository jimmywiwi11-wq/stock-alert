/* Stock Alert V7.70 - Cash Reconciliation */
(function(){
  ...
  function normalize(row){
    ...
    return {...base,...row,id:row?.id||`cash_${branch}_${date}`,type:row?.type||'record',branch,branchId:String(branch),date,
      ...
      sales:row?.sales??row?.salesTotal??'',transfer:row?.transfer??row?.transferTotal??'',note:row?.note||'',savedBy:row?.savedBy||row?.by||userName(),
  ...
  function collectSalesForm(){
    ...
    const base=salesRecord(salesBranch,date);
    return normalize({...base,type:'record',branch:salesBranch,date,sales:document.getElementById('salesOnlyTotalV768')?.value||'',transfer:document.getElementById('salesOnlyTransferV768')?.value||'',note:(document.getElementById('salesOnlyNoteV768')?.value||'').trim(),updatedAt:Date.now(),updatedBy:userName(),savedBy:base.savedBy||userName()});
  }
  ...
  function renderSalesEntry(record=salesRecord(salesBranch,today())){
    ...
    document.getElementById('salesOnlySub').textContent=`${dateText(record.date)} • ${salesSourceLabel(record)}`;
    ...
    const notice=hasCashCheck(record)?'<div class="cashSync wait">วันนี้มีข้อมูลยอดขายจากการตรวจสอบเงินแล้ว</div>':hasSalesData(record)?'<div class="cashSync ok">วันนี้มีข้อมูลยอดขายที่บันทึกไว้แล้ว</div>':'';
    box.innerHTML=`<div class="salesOnlyPanelV768 ${branch.theme}">${connectionNotice()}${notice}<section class="cashInputCard"><label>วันที่</label><input class="input" id="salesOnlyDateV768" type="date" value="${esc(record.date)}" onchange="loadSalesOnlyDate()"><label>ยอดขายรวม</label><small>ยอดขายทั้งหมดของวัน รวมทั้งเงินสดและยอดโอน</small><input class="input cashMoneyInput" id="salesOnlyTotalV768" inputmode="decimal" value="${esc(record.sales||'')}" placeholder="0"><label>ยอดโอน</label><small>ส่วนหนึ่งของยอดขายรวมที่ลูกค้าชำระด้วยการโอน</small><input class="input cashMoneyInput" id="salesOnlyTransferV768" inputmode="decimal" value="${esc(record.transfer||'')}" placeholder="0"><div class="cashFormula salesOnlyCalcV768"><div><span>ยอดเงินสด</span><b id="salesOnlyCashV768">${money(salesCashValue(record))} บาท</b></div></div><label>หมายเหตุ <span class="supplierUnknown">(ไม่บังคับ)</span></label><textarea id="salesOnlyNoteV768" placeholder="ไม่บังคับ">${esc(record.note||'')}</textarea></section><div class="cashActionsV749 salesOnlyActionsV768"><button class="btn ok" id="salesOnlySaveBtnV768" ${salesSaving?'disabled':''} onclick="saveSalesOnlyRecord()">บันทึกยอดขาย</button><button class="btn gray" onclick="go('cashSalesMenuPage')">ย้อนกลับ</button></div></div>`;
    bindSalesOnlyInputs();
  }
  ...
  function bindSalesOnlyInputs(){
    document.querySelectorAll('#salesOnlyBox input,#salesOnlyBox textarea').forEach(el=>{
      el.addEventListener('input',()=>{if(el.classList.contains('cashMoneyInput'))cleanNumericInput(el,false);refreshSalesOnlyCalc();});
      el.addEventListener('blur',()=>{if(el.classList.contains('cashMoneyInput'))formatField(el,false);refreshSalesOnlyCalc();});
    });
  }
  async function cloudMergeSales(record,force=false){
    ...
    const payload={...record,type:'record',branch:Number(record.branch),branchId:String(record.branch),date:record.date,sales:num(record.sales),transfer:num(record.transfer),salesTotal:num(record.sales),transferTotal:num(record.transfer),cashSalesTotal:num(record.sales)-num(record.transfer),source:updateSource,updatedAt:Date.now(),updatedBy:userName(),salesActivityLog:salesLog({...record,source:updateSource},'sales-only-save',updateSource)};
  ...
  window.openSalesEntryCurrent=function(){salesBranch=currentDeviceBranch();go('cashSalesEntryPage');renderSalesEntry(salesRecord(salesBranch,today()));};
  window.loadSalesOnlyDate=function(){const date=document.getElementById('salesOnlyDateV768')?.value||today();renderSalesEntry(salesRecord(salesBranch,date));};
  ...
  function total(list){return list.reduce((a,r)=>{const c=calc(r),checked=hasCashCheck(r),key=`${r.branch}_${r.date}`;a.sales+=c.sales;a.transfer+=c.transfer;a.cash+=c.sales-c.transfer;a.days.add(key);if(checked){a.checkDays.add(key);a.expenses+=c.expenseTotal;a.added+=c.addTotal;a.expected+=c.expected;a.evening+=c.evening;if(c.diff>0){a.over+=c.diff;a.overDays.add(key);}else if(c.diff<0){a.short+=Math.abs(c.diff);a.shortDays.add(key);}else a.okDays.add(key);}return a;},{sales:0,transfer:0,cash:0,expenses:0,added:0,expected:0,evening:0,over:0,short:0,days:new Set(),checkDays:new Set(),shortDays:new Set(),overDays:new Set(),okDays:new Set()});}
  ...
  function salesBranchBreakdownHtml(list){
    ...
    const row=(title,t)=>{const net=t.over-t.short;return `<div class="salesBreakRowV769"><b>${title}</b><span>ยอดขายรวม ${t.days.size?money(t.sales)+' บาท':'ยังไม่มีข้อมูล'}</span><span>ยอดโอน ${t.days.size?money(t.transfer)+' บาท':'ยังไม่มีข้อมูล'}</span><span>ยอดเงินสด ${t.days.size?money(t.cash)+' บาท':'ยังไม่มีข้อมูล'}</span><span>ตรวจสอบเงินจริง ${t.checkDays.size} วัน • ขาด ${money(t.short)} บาท • เกิน ${money(t.over)} บาท • ${netText(net)}</span></div>`;};
  ...
  function salesSummaryMetaHtml(list,mode){
    ...
    if(mode==='yearly'){
      ...
      return `<div class="salesMetaGridV769"><div><span>จำนวนวันที่มีข้อมูลยอดขาย</span><b>${dayCount} วัน</b></div><div><span>จำนวนวันที่มีการตรวจสอบเงินจริง</span><b>${t.checkDays.size} วัน</b></div><div><span>ค่าเฉลี่ยต่อเดือน</span><b>${money(avgMonth)} บาท</b></div><div><span>ค่าเฉลี่ยต่อวันที่มีข้อมูลยอดขาย</span><b>${money(avgDay)} บาท</b></div></div>`;
    }
    if(mode==='monthly')return `<div class="salesMetaGridV769"><div><span>จำนวนวันที่มีข้อมูลยอดขาย</span><b>${dayCount} วัน</b></div><div><span>จำนวนวันที่มีการตรวจสอบเงินจริง</span><b>${t.checkDays.size} วัน</b></div><div><span>ค่าเฉลี่ยต่อวันที่มีข้อมูลยอดขาย</span><b>${money(avgDay)} บาท</b></div></div>`;
    return '';
  }
  function salesDailyHtml(list){
    if(!list.length)return '<div class="empty">ยังไม่มีข้อมูลยอดขาย</div>';
    return `<div class="salesDailyListV769">${list.map(r=>{const c=calc(r);return `<div class="cashHistoryRow salesHistoryRowV768 salesDayCardV769"><button onclick="openSalesDetail(${r.branch},'${esc(r.date)}')"><b>${dateText(r.date)} • ${esc(branches[r.branch]?.name||'')}</b><span class="salesAmountsV769"><strong>ยอดขายรวม ${money(c.sales)} บาท</strong><strong>ยอดโอน ${money(c.transfer)} บาท</strong><strong>ยอดเงินสด ${money(c.cashSales)} บาท</strong></span><span>ที่มา: ${salesSourceLabel(r)} • ผู้บันทึก ${esc(r.updatedBy||r.savedBy||'-')} • แก้ไขล่าสุด ${timeNow(r.updatedAt||r.savedAt||r.createdAt)}</span>${Number(c.sales)===0&&String(r.sales).trim()!==''?'<span class="salesZeroTagV768">บันทึกยอดขายเป็น 0 จริง</span>':''}</button><button class="cashMoreBtn" onclick="openSalesHistoryMenu(${r.branch},'${esc(r.date)}')">⋮</button></div>`;}).join('')}</div>`;
  }
  ...
  function salesYearlyHtml(list){
    ...
    return `<input class="input salesPeriodInputV769" type="number" id="cashYearFilter" value="${y}" min="2000" max="2100" onchange="setSalesYearFilter(this.value)">${salesDashboardHtml(yearRows)}${salesSummaryMetaHtml(yearRows,'yearly')}${reconciliationSummaryHtml(yearRows,'yearly')}${salesBranchBreakdownHtml(yearRows)}<div class="sectionTitle">สรุปแยกตามเดือน</div>${Object.keys(grouped).sort().map(k=>{const t=total(grouped[k]),net=t.over-t.short;return `<div class="miniItem salesMonthRowV769"><div><b>${esc(k)}</b><div class="meta">ตรวจสอบ ${t.checkDays.size} วัน • ขาดรวม ${money(t.short)} บาท • เกินรวม ${money(t.over)} บาท • ${netText(net)}</div></div><span>${money(t.sales)} บาท</span></div>`;}).join('')||'<div class="empty">ปีนี้ยังไม่มีข้อมูล</div>'}`;
  }
