(function(root){
  ...
  const DESKTOP_HISTORY_BUILD = 'V23-HISTORY-SYNC-20260804-ROOT-FIX';
  ...
  function refreshFirestoreHistoryOnce(options={}){
    return loadUnifiedInvoiceHistory(options);
  }
  ...
  async function markPrinted(invoice, actor={}){
    ...
    if (invoiceId) {
      ...
      await root.db.collection(HISTORY_INDEX_COLLECTION).doc(invoiceId).set(printedInvoice, { merge: true });
      await root.db.collection(LEGACY_INVOICE_COLLECTION).doc(invoiceId).set(printedInvoice, { merge: true });
    }
    if (requestId) {
  ...
  async function syncDesktopManualInvoices(invoices, actor={}){
    ...
    const result = await db.runTransaction(async transaction => {
      ...
      return rows.map(row => {
        ...
        transaction.set(db.collection(HISTORY_INDEX_COLLECTION).doc(invoiceId), central, { merge: true });
