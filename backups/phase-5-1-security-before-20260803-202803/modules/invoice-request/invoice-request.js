(function(){
  ...
  function ensureHomeButton(){
    ...
    button.innerHTML = '<span class="cmsInvoiceIconV42">ภ</span><span><b>สั่งทำใบกำกับภาษี</b><span>สร้างคำขอจริงให้ฝ่ายออกใบกำกับภาษี</span></span><span class="cmsInvoiceArrowV42">›</span>';
  ...
  function renderLanding(){
    ...
    page.innerHTML = [
      ...
      '<button class="cmsInvoiceMenuButtonV42" onclick="CMSInvoiceRequest.openHistory()"><i>3</i><span><b>ประวัติใบกำกับภาษี</b><span>ยังไม่ใช่ Tax Invoice History จริงใน Phase 5.1</span></span><b>›</b></button>',
      '</div>'
    ].join('');
  }
  ...
  function renderStatus(){
    ...
    document.getElementById('cmsInvoiceRequestStatusPageV42').innerHTML = [
      ...
      rows.length ? rows.map(row => `<div class="cmsInvoiceListRowV42"><b>${esc(row.requestNumber || row.requestId)}</b><span class="cmsInvoiceBadgeV42 ${row.testMode ? 'test' : ''}">${row.testMode ? 'TEST' : (row.syncStatus === 'pending' ? 'รอซิงค์' : 'จริง')}</span><span class="cmsInvoiceListMetaV42">ลูกค้า: ${esc(row.customerSnapshot?.customerName || row.customer?.customerName || '-')}<br>ผู้ส่ง: ${esc(row.requestedByNickname || row.sender?.nickname || '-')} / ${esc(row.requestedBranch || row.sender?.branch || '-')}<br>วันเวลา: ${esc(row.requestedAt || row.sender?.requestedAt || '-')}<br>รายการ: ${row.itemCount || 0} | คาดว่า ${row.expectedInvoiceCount || 0} ใบ | ยอด ${money(row.grandTotal || row.subtotalPreview)}<br>สถานะ: ${esc(row.status || 'กำลังดำเนินการ')}</span></div>`).join('') : '<div class="empty">ยังไม่มีคำขอในเครื่องนี้</div>',
      '</div>'
    ].join('');
  }
  ...
  async function addNewProduct(){
    ...
    const created = window.ChokAnanProductMaster && typeof window.ChokAnanProductMaster.createProductAsync === 'function'
      ? await window.ChokAnanProductMaster.createProductAsync({ productName: name, name, unit, salePrice, price: salePrice, createdFrom: 'invoice-request' }, { by: sender().requestedByNickname, nickname: sender().requestedByNickname, branch: sender().requestedBranch })
      : (window.ChokAnanProductMaster && typeof window.ChokAnanProductMaster.createProduct === 'function'
        ? window.ChokAnanProductMaster.createProduct({ productName: name, name, unit, salePrice, price: salePrice, createdFrom: 'invoice-request' }, { by: sender().requestedByNickname, nickname: sender().requestedByNickname, branch: sender().requestedBranch })
        : null);
    state.items.push({
      requestItemId: newId('req-item'),
