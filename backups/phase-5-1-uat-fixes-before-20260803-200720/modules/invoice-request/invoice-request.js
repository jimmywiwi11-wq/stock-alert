(function(){
  ...
  function requestSnapshot(base){
    ...
    return {
      ...
      appVersion: window.APP_VERSION_LABEL || window.APP_VERSION || 'unknown',
  ...
  function addNewProduct(){
    ...
    const created = window.ChokAnanProductMaster && typeof window.ChokAnanProductMaster.createProduct === 'function'
      ? window.ChokAnanProductMaster.createProduct({ productName: name, name, unit, salePrice, price: salePrice, createdFrom: 'invoice-request-production' }, { by: sender().requestedByNickname, branch: sender().requestedBranch })
      : null;
    state.items.push({
      requestItemId: newId('req-item'),
