/* Stock Alert V7.69 - Cash Reconciliation */
(function(){
  ...
  function total(list){return list.reduce((a,r)=>{const c=calc(r);a.sales+=c.sales;a.transfer+=c.transfer;a.cash+=c.sales-c.transfer;a.expenses+=c.expenseTotal;a.expected+=c.expected;a.evening+=c.evening;a.over+=c.diff>0?c.diff:0;a.short+=c.diff<0?Math.abs(c.diff):0;a.days.add(`${r.branch}_${r.date}`);return a;},{sales:0,transfer:0,cash:0,expenses:0,expected:0,evening:0,over:0,short:0,days:new Set()});}
  ...
  function salesBranchBreakdownHtml(list){
    ...
    const row=(title,t)=>`<div class="salesBreakRowV769"><b>${title}</b><span>ยอดขายรวม ${t.days.size?money(t.sales)+' บาท':'ยังไม่มีข้อมูล'}</span><span>ยอดโอน ${t.days.size?money(t.transfer)+' บาท':'ยังไม่มีข้อมูล'}</span><span>ยอดเงินสด ${t.days.size?money(t.cash)+' บาท':'ยังไม่มีข้อมูล'}</span></div>`;
  ...
  function salesSummaryMetaHtml(list,mode){
    ...
    if(mode==='yearly'){
      ...
      return `<div class="salesMetaGridV769"><div><span>จำนวนวันที่มีข้อมูล</span><b>${dayCount} วัน</b></div><div><span>ค่าเฉลี่ยต่อเดือน</span><b>${money(avgMonth)} บาท</b></div><div><span>ค่าเฉลี่ยต่อวันที่มีข้อมูล</span><b>${money(avgDay)} บาท</b></div></div>`;
    }
    if(mode==='monthly')return `<div class="salesMetaGridV769"><div><span>จำนวนวันที่มีข้อมูล</span><b>${dayCount} วัน</b></div><div><span>ค่าเฉลี่ยต่อวันที่มีข้อมูล</span><b>${money(avgDay)} บาท</b></div></div>`;
    return '';
  }
  ...
  function salesYearlyHtml(list){
    ...
    return `<input class="input salesPeriodInputV769" type="number" id="cashYearFilter" value="${y}" min="2000" max="2100" onchange="setSalesYearFilter(this.value)">${salesDashboardHtml(yearRows)}${salesSummaryMetaHtml(yearRows,'yearly')}${salesBranchBreakdownHtml(yearRows)}<div class="sectionTitle">สรุปแยกตามเดือน</div>${Object.keys(grouped).sort().map(k=>{const t=total(grouped[k]);return `<div class="miniItem salesMonthRowV769"><div><b>${esc(k)}</b><div class="meta">${t.days.size} วัน • ยอดโอน ${money(t.transfer)} บาท • ยอดเงินสด ${money(t.cash)} บาท</div></div><span>${money(t.sales)} บาท</span></div>`;}).join('')||'<div class="empty">ปีนี้ยังไม่มีข้อมูล</div>'}`;
  }
  function salesDailySectionHtml(list){
    ...
    return `<input class="input salesPeriodInputV769" type="date" id="cashDateFilter" value="${esc(d)}" onchange="setSalesDateFilter(this.value)">${salesDashboardHtml(dayRows)}${salesBranchBreakdownHtml(dayRows)}${salesDailyHtml(dayRows)}`;
  }
