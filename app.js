...
/* === Stock Alert V7.7 real targeted fixes: small copy buttons + exact/similar product prompt === */
(function(){
  const VERSION='7.7';
  ...
  try{
    renderList=function(){
      ...
      const box=document.getElementById('listBox'); if(!box)return;
      if(listMode==='all'){ let gs=typeof groups==='function'?groups():[]; if(q) gs=gs.filter(g=>(typeof norm==='function'?norm(g.name):cleanName(g.name)).includes(q)); gs=gs.sort((a,b)=>String(a.name).localeCompare(String(b.name),'th')); if(!listDetailed){ box.innerHTML=compact(gs.map(g=>g.name)); return; } box.innerHTML=gs.length?gs.map(g=>groupItemHtml(g)).join(''):'<div class="empty">ยังไม่มีรายการ</div>'; return; }
      const br=listMode==='branch1'?1:2; let arr=getItems().filter(x=>Number(x.branch)===br); if(q) arr=arr.filter(x=>(typeof norm==='function'?norm(x.name):cleanName(x.name)).includes(q)); arr=arr.sort((a,b)=>listDetailed?((b.updatedAt||0)-(a.updatedAt||0)):String(a.name).localeCompare(String(b.name),'th')); box.innerHTML=arr.length?(listDetailed?arr.map(x=>itemHtml(x)).join(''):compact(arr.map(x=>x.name))):'<div class="empty">ยังไม่มีรายการ</div>';
    }; window.renderList=renderList;
