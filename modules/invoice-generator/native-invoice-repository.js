(function(root){
  ...
  const BUILD = 'V34-VERIFY-PER-INVOICE-IV000110';
  let cache = [];
  let meta = {
    build: BUILD,
    primaryCount: 0,
  ...
  async function refresh(){
    try {
      ...
      return updateCache(primaryRows, primaryRows.length ? migrateLegacyReadOnly() : []);
    } catch (error) {
  ...
  async function markPrinted(invoice, actor={}){
    ...
    if (root.db && typeof root.db.collection === 'function' && printedRow.invoiceId) {
      ...
      if (printedRow.sourceRequestId) {
        const requestRef = root.db.collection('invoiceRequests').doc(String(printedRow.sourceRequestId));
        let printedInvoiceCount = 1;
        let generatedCount = 1;
        let printedInvoiceIds = [printedRow.invoiceId].filter(Boolean);
        let printedInvoiceNumbers = [printedRow.invoiceNumber || printedRow.no].filter(Boolean);
        ...
        if (requestSnap && requestSnap.exists) {
          ...
          generatedCount = ids.length || generatedCount;
          if (ids.length) {
            const invoiceSnaps = await Promise.all(ids.map(id => root.db.collection(PRIMARY_COLLECTION).doc(String(id)).get().catch(() => null)));
            const printedSnaps = invoiceSnaps.filter(snap => snap && snap.exists && ((snap.data() || {}).printed === true || String((snap.data() || {}).printStatus || '').toLowerCase() === 'printed'));
            printedInvoiceCount = printedSnaps.length;
            printedInvoiceIds = printedSnaps.map(snap => snap.id);
            printedInvoiceNumbers = printedSnaps.map(snap => (snap.data() || {}).invoiceNumber || (snap.data() || {}).no || snap.id).filter(Boolean);
          }
        }
        const aggregateStatus = generatedCount > 0 && printedInvoiceCount >= generatedCount ? 'printed' : (printedInvoiceCount > 0 ? 'partially_printed' : 'ready_to_print');
        await root.db.collection('invoiceRequests').doc(String(printedRow.sourceRequestId)).set({
          status: aggregateStatus,
          printStatus: aggregateStatus,
          printedAt: aggregateStatus === 'printed' ? now : null,
          printedBy: aggregateStatus === 'printed' ? (actor.by || actor.uid || 'tax-invoice-desktop') : '',
          printedInvoiceCount,
          printedInvoiceIds,
          printedInvoiceNumbers,
          updatedAt: now
        }, { merge: true });
      }
    }
  ...
  function subscribe(listener){
    ...
    if (!firestoreUnsubscribe && root.db && typeof root.db.collection === 'function') {
      firestoreUnsubscribe = root.db.collection(PRIMARY_COLLECTION).onSnapshot(snapshot => {
        ...
        updateCache(primaryRows, primaryRows.length ? migrateLegacyReadOnly() : []);
      }, error => {
        meta = { ...meta, lastError: error && (error.message || error.code) || String(error) };
      });
    } else {
