(function(root){
  ...
  function ensureControls(){
    ...
    summary.innerHTML = `
      <div class="historyV2Toolbar">
        <div><label>Date</label><input id="historyDateFilterV2" type="date" onchange="renderHistory()"></div>
  ...
  function filters(){
    return {
      ...
      date: root.historyDateFilterV2 && root.historyDateFilterV2.value || '',
  ...
  function renderHistoryV2(){
    ...
    if (diagBox && new URLSearchParams(root.location.search).get('historyDebug') === '1') {
      ...
      diagBox.innerHTML = `<pre class="small" style="white-space:pre-wrap">${esc(JSON.stringify({
        build: BUILD,
        currentInvoiceId: current.invoiceId || current.id || '',
        currentInvoiceNumber: current.no || current.invoiceNumber || '',
        source: current.source || current.sourceCollection || '',
        requestId: current.sourceRequestId || current.requestId || '',
        saveFunction: 'NativeInvoiceRepository.save',
        repositorySource: repo().PRIMARY_COLLECTION,
        saveResult: d.lastSaveResult,
        primaryCount: d.primaryCount,
        legacyCount: d.legacyCount,
        mergedCount: d.mergedCount,
        duplicateCount: d.duplicateCount,
        historyRowFound: !!rows.length,
        lastSaveError: d.lastError,
        lastPrintStatusUpdate: d.lastPrintStatusUpdate
      }, null, 2))}</pre>`;
    }
  }
