...
(async () => {
  ...
  await page.addInitScript(() => {
    ...
    localStorage.setItem('employeeInvoiceRequestsFixture', JSON.stringify([{
      ...
      customerSnapshot: {
        ...
        customerPrefix: 'บริษัท',
        customerName: 'เอ็นอาร์ คอนสตรัคชั่น จำกัด',
        address1: '6/50 หมู่ที่ 8 ต.คูบางหลวง',
        address2: 'อ.ลาดหลุมแก้ว จ.ปทุมธานี 12140',
        customerTaxId: '0105561048928',
        phone: '0810009991',
        headOffice: true,
        branchNumber: '00000'
      },
  ...
  assert.strictEqual(await page.locator('#buyerName').inputValue(), 'บริษัท เอ็นอาร์ คอนสตรัคชั่น จำกัด (สำนักงานใหญ่)');
  assert.strictEqual(await page.locator('#buyerTax').inputValue(), '0105561048928');
  assert.ok((await page.locator('#buyerAddress').inputValue()).includes('6/50 หมู่ที่ 8 ต.คูบางหลวง'));
  assert.ok((await page.locator('#buyerAddress').inputValue()).includes('อ.ลาดหลุมแก้ว จ.ปทุมธานี 12140'));
  ...
  assert.ok((await page.locator('#invoicePreview').textContent()).includes('บริษัท เอ็นอาร์ คอนสตรัคชั่น จำกัด (สำนักงานใหญ่)'));
  assert.ok((await page.locator('#invoicePreview').textContent()).includes('6/50 หมู่ที่ 8 ต.คูบางหลวง'));
  assert.ok((await page.locator('#invoicePreview').textContent()).includes('อ.ลาดหลุมแก้ว จ.ปทุมธานี 12140'));
  assert.ok((await page.locator('#invoicePreview').textContent()).includes('0105561048928'));
  ...
  await page.getByRole('button', { name: 'บันทึกและพิมพ์บิล', exact: true }).click();
  ...
  assert.ok(printSnapshot.text.includes('บริษัท เอ็นอาร์ คอนสตรัคชั่น จำกัด (สำนักงานใหญ่)'));
  assert.ok(printSnapshot.text.includes('6/50 หมู่ที่ 8 ต.คูบางหลวง'));
  assert.ok(printSnapshot.text.includes('อ.ลาดหลุมแก้ว จ.ปทุมธานี 12140'));
  assert.ok(printSnapshot.text.includes('0105561048928'));
  ...
  assert.strictEqual(saved[0].buyerName, 'บริษัท เอ็นอาร์ คอนสตรัคชั่น จำกัด (สำนักงานใหญ่)');
  assert.ok(String(saved[0].buyerAddress || '').includes('6/50 หมู่ที่ 8 ต.คูบางหลวง'));
  assert.ok(String(saved[0].buyerAddress || '').includes('อ.ลาดหลุมแก้ว จ.ปทุมธานี 12140'));
  assert.strictEqual(saved[0].buyerTax, '0105561048928');
  ...
  assert.ok(historyText.includes('บริษัท เอ็นอาร์ คอนสตรัคชั่น จำกัด (สำนักงานใหญ่)'));
